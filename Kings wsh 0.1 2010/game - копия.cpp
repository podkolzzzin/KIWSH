...
public:
    bool just;
    button(coords c,int butHandle,const char* butText,int paddingT=0,int paddingL=0,int hoverHandle=0, int activeHandle=0)
    {
        //if(c.x>=0)
        //{
        crd.x=c.x;
...
void up(player &p, bool &end, int butUp,int hoverUp,int endBut, int endHover)
{
...
{
    SgSelectBrush(BRUSH_SOLID,color);
    SgRectangle(x,y,x+atack,y+30);


}
bool atackF(player &p,characteristics kingdom, int king, int kingAtack, int enemy, int enemyAtack, int king1, int kingAtack1, int enemy1, int enemyAtack1,int &hp)
{
    ...
    int gravity=3;
    int g=10;
    int hodChange=kingHar.power*10;
    bool ifatack=false;
    bool just=false;
    bool at=false;
    bool atEnemy=false;
    ...
    int napr=0;
    int enemyNapr=0;
    int enemyChange=kingdom.power*10;
    ...
    while(hpKing>0 && hpKingdom>0)
    {
        ...
        else
        {
            ...
            if(hodChange==kingHar.power*10)
            {
                ifatack=true;hodChange=kingHar.power*10;
            }
            if(hodChange<kingHar.power*10)
            {
                if(hodChange+kingHar.reakcia/10>=kingHar.power*10)
                    hodChange=kingHar.power*10;
                if(hodChange+kingHar.reakcia/10<kingHar.power*10)
                    hodChange+=kingHar.reakcia/10;
        ...
        if(d>=double(width/1.9) && enemyCoords.x<=kingCoords.x)
        {
            ...
            enemyChange+=kingdom.reakcia/10;
            SgDrawImage(enemy1,enemyCoords.x,enemyCoords.y);
            if(enemyChange>=kingdom.power*10)
            {
                enemyIfAtack=true;            
            }
        }
...
void main()
{
    ...
    SgCreate(1250,800, "KINGS:Wall Street Hotel 0.1");
    ...
    for(int i=0;i<17;i++)
    {
        for(int j=0;j<13;j++)
        {
            h.x=i;
            h.y=j;
            wsh.setContent(h,rand()%100+25,15,15);           
        }    
    }
    /////////////// Подгрузка главной кнопки
    int mainButHandle=SgLoadImage("Main.jpg");
       ...
       // wsh.overline(over1,5,sgRGB(0,255,0),true);
        ...
        if(atack.drawButton())
        {
            ...
            if(nap.x>=0 && nap.y>=0)
            {
                ...
                if(atackF(p1,deff,king,kingAtack,enemy,enemyAtack,king1,kingAtack1,enemy1,enemyAtack1,hpNap))
                {
                    p1.setHp(hpNap);
        ...
        if(upBool)
        {
            up(p1,upBool,u,u1,mainButHandle,mainButHandleHover); 
        }
